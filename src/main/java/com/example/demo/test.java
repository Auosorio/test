    ...
    private void loadTasks() {
        ...
        if (selectedFile != null) {
            try (BufferedReader reader = new BufferedReader(new FileReader(selectedFile))) {
                ...
                for (int i = 0; i < taskLines.size() && i < taskInputs.length; i++) {
                    taskInputs[i].setText(taskLines.get(i));
                }
...
/*src/main/java/com/example/demo/test.txt*/
...
/*StringBuilder content = new StringBuilder();
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (!task.isEmpty()) {
                int priority = prioritySpinners[i].getValue();
                content.append("Priority ").append(priority).append(": ").append(task).append("\n");
            }
        }*/